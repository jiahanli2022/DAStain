...
def get_transform(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True, useBlur=False):
    transform_list = []
    if grayscale:
    ...
    if useBlur:
        transform_list += [transforms.GaussianBlur(kernel_size=(opt.kernel_size, opt.kernel_size), sigma=(0.1, 2.0)),]
